...
### 1. 目录设置
主要用于设置下载的图片和 NFO 信息的位置。建议手动新建一个 HomeMovie 文件夹，并在里面创建两个子文件夹：
...
“影片库目录”选择 `Library`，“STRM 保存位置”选择 `STRM`。两个目录不要混选；批量入库会先把 STRM 写入 `STRM`，刮削后再把整理结果写入 `Library`。
...
### 5. 使用
115 网盘登录以后，可以在网盘界面选择想要添加的视频，会先使用设置中的首选来源刮削；失败时自动回退到其它来源，然后在影片页面就能看到刚刚刮削的影片。
